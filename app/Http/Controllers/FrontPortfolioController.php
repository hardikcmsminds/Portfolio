...
class FrontPortfolioController extends Controller{
    ...
    public function show($token){
        ...
        if($token_new->status == 1){
            if($token_new->technology == 'all'){
                $portfolio = DB::select('select * from portfolio WHERE status="1"');
            }else{
                $portfolio = DB::table('accesstoken')->leftJoin('portfolio', 'accesstoken.technology', '=', 'portfolio.technology')->where('accesstoken.accesstoken',$token)->where('accesstoken.status',1)->get();
            }
             if(!empty($portfolio)){
    ...
    public function singledetails($token){
        $portfolio = DB::select('select * from portfolio where status = "1" AND portfolioid = '.$token);
    ...
    public function ajaxloadmore() {
        if($_POST['id'] !=0){
            ...
            if($_POST['catid'] !=""){
                 $catid = $_POST['catid'];
                 //echo 'test';
                 $portfolio_count = DB::select("select count(*) as num_rows from portfolio WHERE status = '1' AND technology= ".$catid."");
                 $allNumRows = $portfolio_count[0]->num_rows;
                $portfolio1 = DB::select("select * from portfolio WHERE status = '1' AND display_index > ".$_POST['id']." AND  technology= ".$catid." ORDER BY display_index ASC LIMIT $showLimit");
            } else{
                //echo 'try';
                 $portfolio_count = DB::select("select count(*) as num_rows from portfolio WHERE status = '1' ORDER BY display_index ASC");
                 $allNumRows = $portfolio_count[0]->num_rows;
                 $portfolio1 = DB::select("select * from portfolio WHERE status = '1' AND display_index > ".$_POST['id']." ORDER BY display_index ASC LIMIT $showLimit");
            }
            ...
            foreach ($portfolio1 as $portfolio) {
                ...
                <div class='hover ehover10'>
                    <img class='img-responsive' src=". (($portfolio->image and file_exists('template/frontend/images/'.$portfolio->image)) ? URL::asset('template/frontend/images/'.$portfolio->image) : asset('template/frontend/images/no-image-icon-15.png')) ." alt='Title img' />
                    <div class='point'>
                        <p>
                            <a data-fancybox='images' href=". (($portfolio->image and file_exists('template/frontend/images/'.$portfolio->image)) ? URL::asset('template/frontend/images/'.$portfolio->image) : asset('template/frontend/images/no-image-icon-15.png')) ." data-caption='Hitt'>
                                <i class='zoom'></i>
                            </a>
                        </p>
                    </div>
    ...
    public function filterData(Request $request) {
        ...
        if($technology !="" && $technology!='undefined' && $technology!='all'){
            $where .= ' and technology='. $technology;
        }
        if(isset($category) && $category!='undefined' && $category!=""){
            $category_list = implode(' OR ', array_map(function($x) { return "FIND_IN_SET('$x', category)"; }, explode(',', $category)));
            $where .= " and ($category_list)";
            // $where .= ' and category IN('. $category.')';
        }
        if(isset($tag) && $tag!=""){
            $tag_list = implode(' OR ', array_map(function($x) { return "FIND_IN_SET('$x', tags)"; }, explode(',', $tag)));
            $where .= " and ($tag_list)";
            // $where .= ' and tags IN('. $tag.')';
        }
        if(isset($theme) && $theme!=" "){
            $theme_list = implode(' OR ', array_map(function($x) { return "FIND_IN_SET('$x', theme)"; }, explode(',', $theme)));
            $where .= " and ($theme_list)";
            // $where .= ' and theme IN('. $theme.')';
        }
        if(isset($package) && $package!=" "){
            $package_list = implode(' OR ', array_map(function($x) { return "FIND_IN_SET('$x', js_css_packages)"; }, explode(',', $package)));
            $where .= " and ($package_list)";
            // $where .= ' and js_css_packages IN('. $package.')';
        }
        if(isset($multitheme) && $multitheme!=" "){
            $where .= ' and multi_theme='. $multitheme;
        }
        if(isset($multisite) &&  $multisite!=" "){
            $where .= ' and multi_sites='. $multisite;
        }
        if(!empty($where)){
            $portfolio = DB::select("select * from portfolio WHERE  ".$where." ORDER BY display_index ASC");
        }else{
            $portfolio = DB::select("select * from portfolio WHERE status = '1' ORDER BY display_index ASC");
        }
